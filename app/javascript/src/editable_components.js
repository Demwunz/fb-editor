...
class EditableContent extends EditableElement {
   ...
   *    so we're trying to get rid of it. See https://github.com/showdownjs/showdown/issues/700
   *
 ...
 * Expected (minimum) frontend structure
 * -------------------------------------
 * <div class="fb-editable" data-fb-content-id="foo" data-fb-content-type="select" data-fb-conent-data=" ...JSON... ">
 *
 * </div>
 **/
class EditableSelectFieldComponent extends EditableCollectionFieldComponent {
  constructor($node, config) {
    ...
    this._preservedItemCount = 2; // Preserves minimum item count based on config
    this.#createInputFieldItems(config);
  ...
  // Override set content to ensure data items are properly stored
  get content() {
    return super.content;
  }
  ...
  // Method to create input fields based on the provided HTML structure and config data
  #createInputFieldItems(config) {
    const $wrapper = $('<fieldset class="govuk-fieldset govuk-fieldset__input-group"></fieldset>');
    this.$node.append($wrapper);

    // Initialize items based on config
    this.items = this.#createEditableCollectionItems(config);

    this.items.forEach((item, index) => {
      const idx = index + 1
      const $inputLabel = $('<label class="govuk-label"></label>')
        .val(item.label)
        .attr('for', `input-label-${idx}`)
        .text(`Option ${idx}`);
      const $inputField = $('<input type="text" class="govuk-input"></input>')
        .val(item.value || '')
        .prop('id', `option-${idx} `);

      const $itemWrapper = $('<div class="govuk-form-group item-wrapper"></div>')
        .append($inputLabel, $inputField);

      $wrapper.append($itemWrapper);

      // Add blur event to update the item in `this.items` when user inputs change
      $([$inputLabel, $inputValue]).on('blur', () => this.#updateItem(item, $inputLabel.val(), $inputValue.val()));
    });
  }

  // Method to create the template for items based on the provided HTML and config
  #createCollectionItemTemplate(config) {
    const $clone = this.$node.find(config.selectorCollectionItem).eq(0).clone();
    const itemConfig = mergeObjects({}, config.data.items[0]);
    $clone.data("config", itemConfig);

    return $clone;
  }

  // Initialize collection items with the same HTML structure and classes
  #createEditableCollectionItems(config) {
    const items = [];
    const component = this;

    this.$node.find(config.selectorCollectionItem).each(function (index) {
      const itemConfig = mergeObjects({}, config.data.items[index]);
      const item = new EditableComponentCollectionItem(component, $(this), itemConfig);
      items.push(item);
    });
    return items;
  }

  // Update item data on blur, preserving project-specific structure
  ...
  // Save method to store updated items in `data.items` while preserving structure
  save() {
    this.data.items = this.items.map(item => ({
      _id: item._id,
      label: item.label,
...
module.exports =  {
  ...
  EditableSelectFieldComponent: EditableSelectFieldComponent,
